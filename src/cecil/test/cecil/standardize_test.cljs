...
(deftest standardize-rearranges-the-query
   (letfn [(sql-lines [s]
     ...
     (test-standardize "SELECT DISTINCT t.columna , t.columnb , t.columnc FROM table t"
       ["SELECT DISTINCT"
        "    t.columna"
        "    , t.columnb"
        "    , t.columnc"
     ...
     (test-standardize "SELECT t.tableid , ti.tableinfoid , tv.version FROM table t INNER JOIN tableinfo ti ON t.tableid = ti.tableid INNER JOIN tableversion tv ON ti.tableinfoid = tv.tableinfoid AND tv.activeind = 1"
       ["SELECT"
        "    t.tableid"
        "    , ti.tableinfoid"
        "    , tv.version"
